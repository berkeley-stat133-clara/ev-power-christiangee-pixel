...
energy_clean <- unit = str_extract(energy_sources, "\\b[MGk]Wh\\b")

```

e.  Discussion: Why might regex cleaning be important before performing joins or aggregations?\
    
    \ Clear out blank space, parenthese, and sort them in clumns so it's easier to access later and join
...
inner <- dim(inner)(regions, by = "state")
...
anti_join(x = states, y = regions, by "state")

```

d.  Discussion: Why would you prefer a left join over an inner join when merging state-level datasets?\
    \ inner_join is no matching -> excluded and left_join -> not excluded, still kept
e. Use pivot_longer() to make the socioeconomic data longer.\

```{r}

pivot_longer(states == ("Income", "Illiteracy", "Population"), names_to = "state", values_to = "value")
...
```{r}
sales <- pivot_longer(sales, cols = c("Jan_Sales", "Feb_Sales", "Mar_Sales"), names_to = "Month")
```
...
```{r}
pivot_wider(sales, names_from = "Month")
```
...
b.  Swap Income for Life Exp to visualize life Illiteracy instead. Try changing the color scale using scale_fill_gradient() or scale_fill_gradient2()

```{r}

ggplot(us_map_joined, aes(long_lat,groupsgroup, fill = "Life Illiteracy"))
